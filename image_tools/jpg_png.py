...
def to_png():
    ...
    save_folder = "./已转换格式"
    if file_path == "":
        tkinter.messagebox.showerror("Error", "请先选择文件路径")
        return
...
def init_interface(src_root=None, src_canvas: Canvas = None):
    ...
    # 选择文件路径的按钮
    browse_button = ttk.Button(root, text="选择图片路径", command=get_dir, width=width, style="success outline button")
    canvas_png.create_window(250, 150, window=browse_button)

    # 开始转换的按钮
    browse_button = ttk.Button(root, text="开始转换", command=to_png, width=width, style="success outline button")
